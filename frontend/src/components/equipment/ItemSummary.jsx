  ...
  return (
    <section className="card p-3.5 space-y-2">
      <div className="flex items-start justify-between gap-2">
        <div>
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-xs font-mono text-gray-500">{item.kodeItem || item.kodeEquipment}</span>
          </div>
      ...
      <div className="text-xs text-gray-600 border-t border-gray-100 pt-2 space-y-1">
        <p><span className="font-medium text-gray-800">Lokasi:</span> {item.lokasi} {item.detailLokasi ? `(${item.detailLokasi})` : ''}</p>
        {item.exp ? (
